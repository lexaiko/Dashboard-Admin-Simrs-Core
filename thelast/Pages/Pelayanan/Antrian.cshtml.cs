...
namespace thelast.Pages.Pelayanan
{
    public class AntrianModel : PageModel
    {
        public string errorMessage = "";
        public string successMessage = "";
        public List<PasienInfo> listPasien = new List<PasienInfo>();

        public void OnPost(string id_pasien)
        {
            try
            {
                string connectionString = "Data Source=.\\sqlexpress;Initial Catalog=simrs;Integrated Security=True;";

                if (!string.IsNullOrEmpty(id_pasien))
                {
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();

                        DateTime tgl_pulang = DateTime.Now; // Waktu lokal perangkat saat tombol ditekan

                        // Update status di tabel TM_PASIEN
                        string updateStatusQuery = "UPDATE TM_PASIEN SET status = 0 WHERE id = @id_pasien";
                        ...
                        // Tambahkan tgl_pulang ke tabel TT_PASIENIGD
                        string updateTglPulangQuery = "UPDATE TT_PASIENIGD SET tgl_pulang = @tgl_pulang WHERE id_pasien = @id_pasien";

                        using (SqlCommand tglPulangCommand = new SqlCommand(updateTglPulangQuery, connection))
                        {
                            tglPulangCommand.Parameters.AddWithValue("@id_pasien", id_pasien);
                            tglPulangCommand.Parameters.AddWithValue("@tgl_pulang", tgl_pulang);
                ...
                else
                {
                    ...
                    errorMessage = "id_pasien kosong atau null";
                }
            }
            catch (Exception ex)
            {
